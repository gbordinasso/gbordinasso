# 👋 Olá, eu sou o Gustavo Bordinasso

Desenvolvedor em formação, cursando Análise e Desenvolvimento de Sistemas  
Focado no desenvolvimento de aplicações e construção de soluções eficientes  

---

## 👤 Sobre mim

Sou o Gustavo Bordinasso, desenvolvedor em formação com foco no desenvolvimento de aplicações.

Tenho experiência prática com lógica de programação, orientação a objetos e integração com banco de dados, atuando na construção de soluções funcionais.

Busco sempre desenvolver soluções eficientes e bem estruturadas, com o objetivo de evoluir constantemente e construir aplicações cada vez melhores.

---

## 🛠️ Tecnologias

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![MySQL](https://img.shields.io/badge/MySQL-00758F?style=for-the-badge&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white)

---

## 📫 Contato

🔗 LinkedIn: https://www.linkedin.com/in/gustavobordinasso/

---

Sempre buscando evoluir como desenvolvedor e construir soluções cada vez melhores.
